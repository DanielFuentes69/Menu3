...
$opcionesBusqueda = [];
$opcionesBusqueda["nombrecategoria"] = "Nombre";
?>
                    ...
                    if ($cantidad_filas > 0) {
                        ...
                        <div class="table-responsive">
                            <table id="melleva" class="table table-bordered">
                                <thead>
                                    <tr>
                                        <th width="50%">Categorías</th>
                                        <th width="20%" class="text-center">Estado</th>
                                        <th width="20%" class="text-center">Productos</th>
                                ...
                                <tbody>
                                    ...
                                    $controller = "Modules_Tienda_Controllers_CategoriasController";
                                    foreach ($filas as $indice => $campo) {
                                        $id_fila = $campo["codcategoria"];
                                        $nombreCategoria = $campo["nombrecategoria"];
                                        $estadoCategoria = $DOM["ESTADOCATEGORIA"][$campo["estado"]];
                                        $cantidadProductos = $campo["cantproductos"];

                                        $estiloEstadoCategoria = " class=\"text-success\"";
                                        if ($campo["estado"] != $DOM["ESTADOCATEGORIA_TXT"]["ACTIVO"]) {
                                            $estiloEstadoCategoria = " class=\"text-muted\"";
                                        }

                                        $Url->add("action", "eliminar");
                                        $Url->add("controller", $controller);
                                        $Url->add("codcategoria", $id_fila);
                                        ...
                                        $xhtml .= "<td{$estiloEstadoCategoria}>{$estadoCategoria}</td>";
                                        $xhtml .= "<td class=\"text-center\"><span style=\"font-size: 13px;\" class=\"badge badge-success\">{$cantidadProductos}</span></td>";
                                        ...
                                        if ($cantidadProductos == 0 && $perfilUsuario == $DOM["PERFILES"]["ADMINISTRADOR"]) {
