...
public function __construct($total, $limit_numrows, $from, $Parameters){
    ...
    if ($this->_numberPages > MAX_NUM_BOXES){
        ...
        $more = "<a class='btn u-btn-blue' title='{$title}' data-container='body' data-placement='right' data-toggle='popover' href='#' data-html='true' data-content=\"{$links}\">Ir a</a>\n";
    }
    if ($total >= $this->_limitNumrows){
        ...
        for ($i = $first; $i <= $last; $i++){
            ...
            if ($counter == $from){
                $this->_currentPage = $i;
                $class = " class=\"btn u-btn-bluegray active\"";
            }
...
private function generateShortcuts(){
    ...
    $xhtml.= "          <input class='u-btn-blue' type='submit' name='btngp' id='btngp' value='Ir' />";
    $xhtml.= "      </div>";
    $xhtml.= "  </div>";
