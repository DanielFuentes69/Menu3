...
public function createThumbnail($rate){
    $filename = $pathFile = $this->_folder."/".$this->_hiddName;
    list($Width, $High) = getimagesize($filename);
    if ($Width<185 && $High<225){
        $rate = 0.95;
    }
    echo "Ancho -> ".$Width;
    echo "alto -> ".$High;
    ...
    $imagen_p = imagecreatetruecolor($newWidth, $newHigh);
    if ($this->_mimetype === "image/png"){
        $imagen = imagecreatefrompng($filename);
    }
    if ($this->_mimetype === "image/gif"){
        $imagen = imagecreatefromgif($filename);
    }
    if ($this->_mimetype === "image/jpeg"){
        $imagen = imagecreatefromjpeg($filename);
    }
    imagecopyresampled($imagen_p, $imagen, 0, 0, 0, 0, $newWidth, $newHigh, $Width, $High);

    imagepng($imagen_p, $filename.".".$this->_ext);
    imagedestroy($imagen);
