...
public function add_item($text, $link = "", $disable= FALSE, $body = "", $level = 1){
...
public function dynamicLoad($text){
    foreach($this->_items as $index => $item){
        if ($item["text"] == $text){
            if ($this->_selectedIndex == $index){
                return true;
...
public function show(){
    $xhtml = "<div style=\"background: #ffffff\">&nbsp;</div>";
    $xhtml = "<ul class=\"nav nav-tabs\" id=".$this->_id.">\n";
    foreach($this->_items as $index => $item){
        $step = "#step_".$index;
        $toggleClass = "data-toggle=\"tab\"";
        if (!empty($this->_items[$index]["link"])){
            $toggleClass = "";
            $step = $this->_items[$index]["link"];
        }
        if ($item["isparent"] == 1){
            ...
            if (empty($item["parent"])){
                ...
                if ($item["disable"]){
                    ...
                    $xhtml.= "<li class=\"".$class."\"><a {$toggleClass} href=\"".$step."\">".$item["text"]."</a></li>\n";                    
                }
            }
        }
    }
